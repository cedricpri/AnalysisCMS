...
TString masspoint = "ttDM0001scalar00010";

TString inputdir  = Form("rootfiles_%s", masspoint.Data());
TString outputdir = Form("figures_%s/", masspoint.Data());
...
void PlotterPrepostfits(TString option = "hist", TString signal = "ttDM0001scalar00010" ){
  ...
  masspoint = signal;
  inputdir  = Form("rootfiles_%s", masspoint.Data());
  outputdir = Form("figures_%s/", masspoint.Data());
  ...
  // Get the data
  //----------------------------------------------------------------------------
  plotter.AddData(Form("01_Data_Full2016_%s", masspoint.Data()), "Data", color_Data);
  ...
  plotter.AddProcess("14_VV",             "Diboson",        color_VZ,       roc_background, -999 );
  plotter.AddProcess("07_ZJets",          "DY",         color_ZJets,     roc_background, -999 );
  plotter.AddProcess("09_TTV",            "tt+V",        color_TTV,       roc_background, -999 );
  ...
  plotter.AddProcess("07_ZJets",     "DY",         color_ZJets, roc_background, -999);
  plotter.AddProcess("03_VZ",        "Diboson",    color_VZ, roc_background, -999);
  plotter.AddProcess("00_Fakes",     "W+jets / t#bar{t}(1l) / tW(1l)", color_Fakes, roc_background, -999);  // Don't lumi scale
  plotter.AddProcess("09_TTV",       "t#bar{t}+V",       color_TTV,roc_background, -999);
  plotter.AddProcess("05_ST",        "Single t (2l)",         color_ST, roc_background, -999);
  plotter.AddProcess("04_TTTo2L2Nu", "t#bar{t}(2l)",         color_TTTo2L2Nu, roc_background, -999);

  // Add signals
  //----------------------------------------------------------------------------

  plotter.AddPrefit("18_Prefit", "pre-fit", kBlue+1);
  //plotter.AddSignal(masspoint.Data(), "S M_{#Phi}=500 GeV, M_{#chi}=1 GeV", color_Signal);

  //plotter.AddSignal("ttDM0001scalar00010", "S m_{#chi}1 m_{S}10 x20"  ,  color_Signal, roc_signal, 20.);
  //plotter.AddSignal("ttDM0001scalar00020", "S m_{#chi}1 m_{S}20 x20"  ,  color_Signal, roc_signal, 20.);
  //plotter.AddSignal("ttDM0001scalar00050", "S m_{#chi}1 m_{S}50 x20"  ,  color_Signal, roc_signal, 20.);
  //plotter.AddSignal("ttDM0001scalar00100", "S m_{#chi}1 m_{S}100 x50"  ,  color_Signal, roc_signal, 50.);
  //plotter.AddSignal("ttDM0001scalar00200", "S m_{#chi}1 m_{S}200 x50"  ,  color_Signal, roc_signal, 50.);
  //plotter.AddSignal("ttDM0001scalar00300", "S m_{#chi}1 m_{S}300 x50"  ,  color_Signal, roc_signal, 50.);
  //plotter.AddSignal("ttDM0001scalar00500", "S m_{#chi}1 m_{S}500 x100"  ,  color_Signal, roc_signal, 100.);
  //plotter.AddSignal("ttDM0001pseudo00010", "PS m_{#chi}1 m_{S}10 x20"  ,  color_Signal, roc_signal, 20.);
  //plotter.AddSignal("ttDM0001pseudo00020", "PS m_{#chi}1 m_{S}20 x20"  ,  color_Signal, roc_signal, 20.);
  //plotter.AddSignal("ttDM0001pseudo00050", "PS m_{#chi}1 m_{S}50 x20"  ,  color_Signal, roc_signal, 20.);
  //plotter.AddSignal("ttDM0001pseudo00100", "PS m_{#chi}1 m_{S}100 x50"  ,  color_Signal, roc_signal, 50.);
  //plotter.AddSignal("ttDM0001pseudo00200", "PS m_{#chi}1 m_{S}200 x50"  ,  color_Signal, roc_signal, 50.);
  //plotter.AddSignal("ttDM0001pseudo00300", "PS m_{#chi}1 m_{S}300 x50"  ,  color_Signal, roc_signal, 50.);
  //plotter.AddSignal("ttDM0001pseudo00500", "PS m_{#chi}1 m_{S}500 x100"  ,  color_Signal, roc_signal, 100.);
  //plotter.AddSignal("ttDM0001scalar00500", "m_{#chi}1 m_{S}500 x1e4",  color_Signal, roc_signal,10000./2.391);
  ...
  // -------------------------------------------------------------------------------------------------------------------------
  //plotter.Draw( "darkpt"         , "(reconstructed) mediator p_{T}",      5, 0, "GeV",  scale, false, -100, 800, ymin);//, 1e4 );
  ...
  //plotter.Draw( "dphillmet"      , "#Delta#phi(" +sll + "," + sm + ")",  5, 2, "rad",  scale, true, 0, 3.2, ymin);//, 1e4);
  ...
  //plotter.Draw( "metPfType1"     , sm,                                  10, 0, "GeV",  scale, true, 0,  200, ymin );//, 1e4);
  //plotter.Draw( "mt2lblb"        , "M_{T2}(" + sl + "b" + sl + "b)",    10, 0, "GeV",  scale, false, 0, 600);
  //plotter.Draw( "mt2ll"          , "M_{T2}(" + sll + ")",               10, 0, "GeV",  scale, false, 0, 200, ymin);//, 1e5); 
